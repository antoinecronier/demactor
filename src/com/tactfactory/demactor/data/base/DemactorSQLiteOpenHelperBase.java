...
/**************************************************************************
 * DemactorSQLiteOpenHelperBase.java, demactor Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
import com.tactfactory.demactor.data.UserSQLiteAdapter;
 ...
 */
public class DemactorSQLiteOpenHelperBase extends SQLiteOpenHelper {
     ...
     */
    public DemactorSQLiteOpenHelperBase(final android.content.Context ctx,
    ...
    public void onCreate(final SQLiteDatabase db) {
        ...
        if (!assetsExist) {
            ...
            if (DemactorApplication.DEBUG) {
                android.util.Log.d(TAG, "Creating schema : Jockey");
            }
            db.execSQL(JockeySQLiteAdapter.getSchema());

            if (DemactorApplication.DEBUG) {
                android.util.Log.d(TAG, "Creating schema : Poney");
            }
            db.execSQL(PoneySQLiteAdapter.getSchema());

            if (DemactorApplication.DEBUG) {
                android.util.Log.d(TAG, "Creating schema : PoneytoJockey");
            }
            db.execSQL(PoneytoJockeySQLiteAdapter.getSchema());

            if (DemactorApplication.DEBUG) {
                android.util.Log.d(TAG, "Creating schema : Score");
            }
            db.execSQL(ScoreSQLiteAdapter.getSchema());
    ...
    public static void clearDatabase(final SQLiteDatabase db) {
        ...
        db.delete(JockeyContract.TABLE_NAME,
                null,
                null);
        db.delete(PoneyContract.TABLE_NAME,
                null,
                null);
        db.delete(PoneytoJockeyContract.TABLE_NAME,
                null,
                null);
        db.delete(ScoreContract.TABLE_NAME,
                null,
                null);
