/**************************************************************************
 * HarmonyFragmentActivity.java, demactor Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
 ...
 *
 **************************************************************************/
package com.tactfactory.demactor.harmony.view;
...
import com.tactfactory.demactor.menu.DemactorMenu;
import com.tactfactory.demactor.DemactorApplication;
import com.tactfactory.demactor.DemactorApplicationBase.DeviceType;
...
public abstract class HarmonyFragmentActivity extends ActionBarActivity {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        try {
            DemactorMenu.getInstance(this).clear(menu);
            DemactorMenu.getInstance(this).updateMenu(menu, this);
        } catch (Exception e) {
            e.printStackTrace();
            result = false;
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        boolean result;
        try {
            result = DemactorMenu.getInstance(this).dispatch(
                                                                    item, this);
        } catch (Exception e) {
            e.printStackTrace();
            result = false;
    ...
    protected void onActivityResult(int requestCode, int resultCode,
                                                                  Intent data) {
        try {
            DemactorMenu.getInstance(this).onActivityResult(
    ...
    public boolean isDualMode() {
        return DemactorApplication.getDeviceType(this).equals(DeviceType.TABLET);
    }
}
